...
document.querySelectorAll('.more_info').forEach(button => {
...
document.querySelectorAll('.less_info').forEach(button => {
    button.addEventListener('click', function() {
        const infoText = this.parentNode;
        infoText.style.display = 'none';
        this.parentNode.previousElementSibling.style.display = 'inline';
    });
});
...
courseclicked.forEach(course => {
    course.addEventListener('click', (event) => {
        ...
        if (!currentState || currentState.pageId !== target_course) {
            ...
            history.pushState({ pageId: target_course }, '', path); 
        }
...
function displaycourse() {
    ...
    if (course_element) {
        if (width < 768) {
            ...
            course_element.style.width = '100%';
        }
    }
}
...
checkAuthStatus().then(user => {
  if(user){
  if (user.email  === 'clementmburumwangi@gmail.com') {
  document.querySelector('#admin-button').style.display = 'flex';
  console.log('admin');
} else {
...
function toggleEditProfile() {
    ...
    document.getElementById('email').value = document.querySelector('.user_email_display').textContent;
    document.querySelector('.profile-body').querySelectorAll('input').forEach(input=>{
      input.disabled = false;
    });
    document.querySelector('.option_command').style.display='block';
...
function cancelEdit() {
    ...
    document.querySelector('.option_command').style.display='none';
        document.querySelector('.profile-body').querySelectorAll('input').forEach(input=>{
      input.disabled = true;
    });
}
